...
(function main() {
  try {
    ...
    run(
      "Bundle discount applies to bookkeeping and totals map via adapter",
      () => {
        ...
        const expectedBkBefore = Math.round(
          (PRICING_CONSTANTS.baseMonthlyFee +
            ...
            PRICING_CONSTANTS.industryMultipliers["Professional Services"]
              .monthly,
        );
        assert(
          expectedBkBefore === 550,
          `expected BK before discount 550, got ${expectedBkBefore}`,
        );
        ...
        const after = (result.bookkeeping as any).breakdown
        ...
        assert(
          ui.totalMonthlyFee === result.combined.monthlyFee,
          "ui.totalMonthlyFee mismatch",
        );
        assert(
          ui.totalSetupFee === result.combined.setupFee,
          "ui.totalSetupFee mismatch",
        );
      },
    );
    ...
    run("QBO fee is 60 and not discounted in bundle", () => {
      ...
      assert(
        typeof pkgDisc === "number" && pkgDisc > 0,
        "packageDiscountMonthly missing or zero",
      );
    ...
    run("Bookkeeping setup fee is computed from formula and positive", () => {
      ...
      assert(
        result.bookkeeping.setupFee > 0,
        "bookkeeping.setupFee should be > 0",
      );
    });
