...
(function main() {
  try {
    ...
    run("Bundle discount applies to bookkeeping and totals map via adapter", () => {
      ...
      assert(bkAfter === expectedBkAfter, `bookkeeping.monthlyFee expected ${expectedBkAfter}, got ${bkAfter}`);
      ...
      assert(before === expectedBkBefore, `breakdown.before expected ${expectedBkBefore}, got ${before}`);
      assert(after === expectedBkAfter, `breakdown.after expected ${expectedBkAfter}, got ${after}`);
    ...
    run("QBO fee is 60 and not discounted in bundle", () => {
      ...
      assert(ui.bookkeeping.monthlyFee === 275, `BK monthly with discount expected 275, got ${ui.bookkeeping.monthlyFee}`);

      // Combined monthly must include qboFee
      const sumOfParts = ui.bookkeeping.monthlyFee + ui.taas.monthlyFee + ui.serviceTierFee + ui.payrollFee + ui.apFee + ui.arFee + (ui as any).qboFee;
      assert(sumOfParts === ui.totalMonthlyFee, `sumOfParts ${sumOfParts} != totalMonthlyFee ${ui.totalMonthlyFee}`);
    });
    ...
    run("Bookkeeping setup fee is computed from formula and positive", () => {
      ...
      assert(result.bookkeeping.setupFee === expectedSetup, `bookkeeping.setupFee expected ${expectedSetup}, got ${result.bookkeeping.setupFee}`);
