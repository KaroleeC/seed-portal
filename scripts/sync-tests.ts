...
async function testCreatePath() {
  ...
  (svc as any).verifyContactByEmail = async (email: string) => ({
    verified: true,
    contact: { id: 'contact-1', properties: { email, firstname: 'Alice', lastname: 'Smith', company: 'ACME' } },
  });
  (svc as any).getOwnerByEmail = async () => 'owner-1';
  (svc as any).createDeal = async () => ({ id: 'deal-1', properties: {} });
  (svc as any).createQuote = async () => ({ id: 'quote-1', title: 'Test Quote' });
  ...
  assert(updatedPayload && updatedPayload.hubspotQuoteId === 'quote-1', 'storage.updateQuote should persist IDs');
...
async function testUpdatePath() {
  ...
  (svc as any).verifyContactByEmail = async (email: string) => ({
    verified: true,
    contact: { id: 'contact-2', properties: { email, firstname: 'Bob', lastname: 'Lee', company: 'Widgets' } },
  });
  (svc as any).getOwnerByEmail = async () => 'owner-2';
  (svc as any).updateDeal = async (dealId: string) => ({ id: dealId, properties: {} });
