...
import { Shield, Users, Settings, Wrench, Lock } from "lucide-react";
    ...
    <div className="settings-dense space-y-3">
      <Tabs value={tab} onValueChange={setTab} className="space-y-3">
        <TabsList className="grid w-full grid-cols-5 h-9">
          ...
          <TabsTrigger value="rbac" className="flex items-center gap-2 h-9 px-3 text-xs">
            <Lock className="w-4 h-4" /> RBAC
          </TabsTrigger>
          <TabsTrigger value="command-dock" className="flex items-center gap-2 h-9 px-3 text-xs">
            <Settings className="w-4 h-4" /> Command Dock
          </TabsTrigger>
