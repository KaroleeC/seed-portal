...
import { Save, RefreshCw, FileText, Link as LinkIcon, DollarSign } from "lucide-react";
...
export default function SeedQCSettingsPanel() {
  ...
  const [included, setIncluded] = useState<Record<string, unknown>>({});
  ...
  useEffect(() => {
    ...
    const nextIncluded: Record<string, unknown> = {};
    for (const svc of SERVICES) {
  ...
  const onSave = () => {
    const item = (local[activeService] ?? { service: activeService }) as ServiceContentItem;
  ...
  const setField = (key: keyof ServiceContentItem, value: string) => {
    setLocal((prev) => {
      const current = (prev[activeService] ?? { service: activeService }) as ServiceContentItem;
      return { ...prev, [activeService]: { ...current, [key]: value } };
    });
  };
    ...
    <PermissionGuard permissions={PERMISSIONS.MANAGE_PRICING} fallback={<div>Access denied</div>}>
      <div className="space-y-3 text-xs">
        ...
        <div className="mb-2 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-white">Calculator Settings</h2>
            <p className="text-white/80">Unified Pricing + SOW configuration</p>
          </div>
        ...
        <Card>
          <CardHeader className="py-3">
            <CardTitle className="text-sm">HubSpot Pipeline</CardTitle>
            <CardDescription className="text-xs">
              Select the HubSpot pipeline and default stage used for creating deals.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end text-xs">
        ...
        <Card>
          ...
          <CardContent className="flex flex-col md:flex-row md:items-center gap-3 text-xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          ...
          <Card>
            <CardHeader className="py-3">
              <CardTitle className="text-sm">SOW</CardTitle>
              <CardDescription className="text-xs">
            ...
            <CardContent className="space-y-2 text-xs">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Use tokens like <code>{"{{companyName}}"}</code>, <code>{"{{monthlyFee}}"}</code>,{" "}
                  <code>{"{{setupFee}}"}</code>, <code>{"{{cleanupMonths}}"}</code>.
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="py-3">
              <CardTitle className="text-sm">Pricing Snapshot</CardTitle>
              <CardDescription className="text-xs">
            ...
            <CardContent className="space-y-2 text-xs">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-base font-semibold text-foreground">
                    {selectedBaseFee ? `$${Number(selectedBaseFee.baseFee).toLocaleString()}` : "—"}
                  </div>
                </div>
              </div>
              <div>
                <div className="text-xs font-medium mb-1.5">Tiers</div>
                <div className="space-y-1.5">
                  {selectedTiers.length === 0 && (
                    <div className="text-xs text-muted-foreground">No tiers for this service.</div>
                  )}
                  {selectedTiers.map((t) => (
                    <div key={t.id} className="border rounded-md p-2 bg-background/50">
                      ...
                      <div className="text-xs text-muted-foreground">
                        Base: ${Number(t.baseFee).toLocaleString()} • Multiplier: {t.tierMultiplier}
                        x
                      </div>
                    </div>
                  ))}
              ...
              <div className="text-[11px] text-muted-foreground">
                For full editing, use the Pricing page. Changes there reflect here after refresh.
              </div>
            </CardContent>
          </Card>
        </div>
