...
export async function fetchQuotes(params: { search?: string; sortField?: string; sortOrder?: QuoteSortOrder }): Promise<Quote[]> {
...
export interface CheckExistingQuotesResponse {
  ...
  data?: {
    hasExistingQuotes: boolean;
    quotes: Quote[];
    verified: Array<{ id: number; hubspotQuoteId?: string | null; existsInHubSpot: boolean }>;
  };
}
