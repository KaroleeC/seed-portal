...
/**
 * Generates email signature HTML from configuration
 */
export function generateSignatureHTML(config: SignatureConfig): string {
  ...
  const logoSizeStyle = logoSizes[logoSize];
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align: top;">
        ...
        ${phone ? `<div style="margin-bottom: 8px; display: flex; align-items: center;">${renderContactIcon("phone")}<a href="tel:${phone}" style="color: ${textColor}; text-decoration: none; font-size: ${sizes.base};">${phone}</a></div>` : ""}
        ${email ? `<div style="margin-bottom: 8px; display: flex; align-items: center;">${renderContactIcon("email")}<a href="mailto:${email}" style="color: ${textColor}; text-decoration: none; font-size: ${sizes.base};">${email}</a></div>` : ""}
        ${website ? `<div style="margin-bottom: 8px; display: flex; align-items: center;">${renderContactIcon("website")}<a href="${website}" style="color: ${textColor}; text-decoration: none; font-size: ${sizes.base};">${website.replace(/^https?:\/\//, "")}</a></div>` : ""}
