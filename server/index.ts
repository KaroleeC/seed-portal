...
(async () => {
  ...
  try {
    ...
    // Allow an explicit override via PORT_OVERRIDE to avoid local conflicts when a
    // platform or secret manager (e.g. Doppler) injects a default PORT=5000.
    // Other ports are firewalled in hosted envs, but for local dev we can override.
    // This serves both the API and the client.
    const portStr = process.env.PORT_OVERRIDE || process.env.PORT || "5000";
    ...
    const host = process.env.HOST || "0.0.0.0"; // Bind to all interfaces for PaaS (Railway/Vercel)
...
process.on("uncaughtException", (error) => {
  ...
  // In development, do not exit for Vite / transform errors to keep the dev server alive
