...
import { processHubSpotQuoteSync, type HubSpotQuoteSyncJobData, type HubSpotQuoteSyncResult } from './hubspot-quote-sync';
...
export async function initializeHubSpotQueue() {
  try {
    ...
    hubspotWorker.on('completed', (job: Job<HubSpotQuoteSyncJobData>, result: HubSpotQuoteSyncResult) => {
      queueLogger.info({ 
        jobId: job.id, 
    ...
    hubspotWorker.on('failed', async (job: Job<HubSpotQuoteSyncJobData> | undefined, err: Error) => {
      if (!job) return;
      ...
      if (job.attemptsMade >= 3) {
        try {
            ...
            blocks: [
              {
                ...
                text: {
                  type: 'mrkdwn',
                  text: `*HubSpot Quote Sync Failed*\n\n` +
          ...
          queueLogger.error({ error: slackError }, 'Failed to send Slack notification for HubSpot sync failure');
        }
      }
    });
...
export async function scheduleQuoteSync(quoteId: number, action: 'create' | 'update', userId: number, priority: number = 1) {
  try {
    if (!hubspotQueue) {
      throw new Error('HubSpot queue not initialized');
    }
    
    const job = await hubspotQueue.add('sync-quote', {
      quoteId,
      action,
      userId
    }, {
    ...
    queueLogger.error({ error, quoteId, action, userId }, '❌ Failed to schedule HubSpot quote sync');
    throw error;
  }
}
