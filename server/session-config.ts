...
async function createRedisClient(redisUrl: string) {
  try {
    ...
    console.error(
      "[SessionConfig] ❌ Failed to import or create Redis client:",
      error,
    );
    throw error;
  }
...
export async function createSessionConfig(): Promise<
  session.SessionOptions & { storeType: string }
> {
  ...
  if (process.env.REDIS_URL) {
    try {
      console.log(
        "[SessionConfig] Attempting Redis connection with REDIS_URL:",
      ...
      await new Promise((resolve, reject) => {
        ...
        redisClient.on("error", (err) => {
          clearTimeout(timeout);
          console.error(
            "[SessionConfig] ❌ Redis error during connection:",
            err,
          );
          reject(err);
        });
      ...
      console.log(
        "[SessionConfig] ✅ Redis session store created successfully",
      );
      console.log("[SessionConfig] Store constructor name:", storeName);
      console.log(
        "[SessionConfig] Store type check:",
        sessionStore instanceof RedisStore,
      );
      ...
      try {
        ...
        console.warn(
          "[SessionConfig] ⚠️ Redis store test failed, but continuing:",
          storeTestError,
        );
  ...
  if (!sessionStore) {
    ...
    console.log(
      "[SessionConfig] ⚠️ Using MemoryStore as fallback. Store type:",
      storeType,
    );
  }
