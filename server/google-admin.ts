      ...
      if (error.code === 403 || error.status === 403) {
        if (error.message?.includes("iam.serviceAccounts.getAccessToken")) {
          ...
          return {
            connected: false,
            error:
              "Domain-wide delegation not configured. Please enable domain-wide delegation for the service account in Google Workspace Admin Console.",
          };
        }
        ...
        return {
          connected: false,
          error:
            "Insufficient permissions. Check admin permissions and domain-wide delegation settings.",
        };
      }
