...
• Ongoing support and consultation
  ...
  private async createInitialServiceLineItems(
    ...
    try {
      ...
      if (!productIds.valid) {
        console.warn(
          "⚠️ Some product IDs may be invalid, but proceeding with line item creation",
        );
      }
        ...
        if (config.shouldInclude) {
          ...
          for (const lineItemConfig of config.lineItems) {
            ...
            if (lineItemConfig.condition) {
              ...
              console.log(
                `✅ Successfully associated ${serviceName} product with quote: $${lineItemConfig.price}`,
              );
            } else {
              console.log(
                `⏭️ Skipping line item ${lineItemConfig.name} - condition not met`,
              );
            }
          }
        } else {
      ...
      if (totalLineItemsCreated === 0) {
        console.warn(
          "⚠️ WARNING: No line items were created! This may indicate a configuration issue.",
        );
      }
    } catch (error) {
...
<a href="https://seedfinancial.io/legal/msa-v-2025-07-01">MSA v2025.07.01</a>`;
  ...
  private async manageServiceLineItems(
    quoteId: string,
    ...
    try {
      ...
      console.log(
        `🔵 Current existing line items: ${existingLineItems.length}`,
      );
      ...
      if (serviceConfig.includesBookkeeping) {
        console.log(
          `✅ Bookkeeping service ENABLED - Adding required line items`,
        );
        if (serviceConfig.bookkeepingMonthlyFee > 0) {
          ...
          console.log(
            `   ✅ Added bookkeeping monthly: $${serviceConfig.bookkeepingMonthlyFee}`,
          );
        }
        if (serviceConfig.bookkeepingSetupFee > 0) {
          ...
          console.log(
            `   ✅ Added bookkeeping setup: $${serviceConfig.bookkeepingSetupFee}`,
          );
        }
      } else {
        console.log(
          `❌ Bookkeeping service DISABLED - Will remove bookkeeping line items`,
        );
      }
      ...
      if (serviceConfig.includesTaas) {
        ...
        if (serviceConfig.taasMonthlyFee > 0) {
          ...
          console.log(
            `   ✅ Added TaaS monthly: $${serviceConfig.taasMonthlyFee}`,
          );
        }
        if (serviceConfig.taasPriorYearsFee > 0) {
          ...
          console.log(
            `   ✅ Added TaaS setup: $${serviceConfig.taasPriorYearsFee}`,
          );
        }
      } else {
      ...
      for (const existingItem of existingLineItems) {
        ...
        for (const [key, requiredItem] of Array.from(
          requiredLineItems.entries(),
        )) {
        ...
        if (!shouldKeep || amount === 0) {
          ...
          if (isServiceItem) {
            ...
            console.log(
              `🗑️ MARKING FOR DELETION: Line item ${existingItem.id}`,
            );
            console.log(`   Product ID: ${productId}`);
            console.log(`   Amount: $${amount}`);
            console.log(
              `   Name: ${existingItem.properties?.name || "Unknown"}`,
            );
            console.log(
              `   Reason: shouldKeep=${shouldKeep}, amount=${amount}`,
            );
          } else {
            console.log(
              `⚠️ KEEPING NON-SERVICE ITEM: ${existingItem.id} (product ${productId})`,
            );
          }
        } else {
          console.log(
            `✅ KEEPING EXISTING ITEM: ${existingItem.id} (product ${productId}, $${amount})`,
          );
        }
      }
      ...
      for (const item of itemsToDelete) {
        console.log(
          `Deleting unnecessary line item: ${item.id} (product ${item.properties?.hs_product_id}, amount $${item.properties?.amount})`,
        );
        try {
      ...
      for (const [key, requiredItem] of Array.from(
        ...
        if (existingItem) {
          const currentAmount = parseFloat(
            existingItem.properties?.amount || "0",
          );
          ...
          if (Math.abs(currentAmount - requiredAmount) >= 0.01) {
            ...
            console.log(
              `Updating existing line item ${existingItem.id}: Product ID ${requiredItem.productId}, $${currentAmount} → $${requiredAmount}`,
            );
            await this.makeRequest(
              `/crm/v3/objects/line_items/${existingItem.id}`,
              {
                method: "PATCH",
            ...
            console.log(
              `✓ Updated existing line item: ${existingItem.id} to $${requiredAmount}`,
            );
          } else {
            console.log(
              `✓ Existing line item already correct: Product ID ${requiredItem.productId}, Amount $${currentAmount}`,
            );
          }
        } else {
          // Create new line item
          console.log(
            `Creating missing line item: ${requiredItem.name} - $${requiredItem.price}`,
          );
          ...
          if (lineItem && lineItem.id) {
            ...
            console.log(
              `Successfully added line item: ${requiredItem.name} - $${requiredItem.price}`,
            );
          }
        }
      }
