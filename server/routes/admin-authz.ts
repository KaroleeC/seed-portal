...
router.get("/api/_authz-check", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    if (!userId) return res.status(400).json({ message: "userId parameter is required or user not authenticated" });
    ...
    res.json({
      ...
      userInfo: {
        ...
        permissions: authzInfo.permissions.map((p: any) => ({ id: p.id, key: p.key, category: p.category })),
      },
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    console.error("Authorization check error:", error);
    res.status(500).json({ message: "Authorization check failed", error: getErrorMessage(error), timestamp: new Date().toISOString() });
  }
});

router.get("/api/_cerbos-explain", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    if (!action || !resourceType) return res.status(400).json({ message: "action and resourceType parameters are required" });
    if (!userId) return res.status(400).json({ message: "userId parameter is required or user not authenticated" });

    const principal = { userId, email: req.user?.email || "unknown", role: req.user?.role };
    ...
    res.json({
      ...
      principal: { id: cerbosPrincipal.id, roles: cerbosPrincipal.roles, departments: cerbosPrincipal.departments, isManager: cerbosPrincipal.isManager },
      explanation,
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    console.error("Cerbos explanation error:", error);
    res.status(500).json({ message: "Failed to get Cerbos decision explanation", error: getErrorMessage(error), timestamp: new Date().toISOString() });
  }
});

router.get("/api/admin/cerbos/policy/:policyName", requireAuth, requireAdmin, async (req, res) => {
  try {
...
router.put("/api/admin/cerbos/policy/:policyName", requireAuth, requireAdmin, async (req, res) => {
  try {
    const { policyName } = req.params;
