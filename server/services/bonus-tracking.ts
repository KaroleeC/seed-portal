...
export class BonusTrackingService {
    ...
    try {
      const milestones = [
        { threshold: 25, amount: 1000, description: "Pro Level - 25 Clients" },
        {
          threshold: 40,
          amount: 5000,
          description: "Expert Level - 40 Clients",
        },
        {
          threshold: 60,
          amount: 7500,
          description: "Master Level - 60 Clients",
        },
        {
          threshold: 100,
          amount: 10000,
          description: "Elite Level - 100 Clients + Equity",
        },
      ];
  ...
  async getPendingBonuses(): Promise<any[]> {
    try {
      ...
      return [
        ...pendingMonthlyBonuses.map((b: any) => ({
          ...b,
          bonusCategory: "monthly",
        })),
        ...pendingMilestoneBonuses.map((b: any) => ({
          ...b,
          bonusCategory: "milestone",
        })),
      ];
    } catch (error) {
