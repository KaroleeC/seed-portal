...
export function createDealsService(
  ...
  async function createDeal(
    ...
    try {
      ...
      if (!pipelineInfo) {
        console.error("Could not find HubSpot pipeline or stage (configured or fallback)");
        throw new Error(
      ...
      const dealProperties: any = {
        ...
        ...(quoteData?.internationalFiling !== undefined && {
          international_filing: quoteData.internationalFiling ? "true" : "false",
        }),
    ...
    try {
      ...
      try {
        ...
        if (companyFromQuote) {
          ...
          try {
            const current = await request(`/crm/v3/objects/deals/${dealId}?properties=dealname`, {
              method: "GET",
            });
