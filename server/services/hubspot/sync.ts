...
export function determineMode(action: SyncAction, quote: any): 'create' | 'update' {
...
export async function syncQuoteToHubSpot(
  ...
  storageOverride?: { getQuote: (id: number) => Promise<any | undefined>; updateQuote: (q: any) => Promise<any> },
  options?: { dryRun?: boolean; includeConnectivity?: boolean }
): Promise<SyncResult> {
  const start = Date.now();
  const log = hubspotLogger.child({ op: 'syncQuoteToHubSpot' });
  log.info({ quoteIdInput, action, actorEmail, dryRun: options?.dryRun }, '🔄 HubSpot quote sync started');
  appendModuleLog('hubspot', 'info', 'HubSpot sync started', { quoteIdInput, action, actorEmail, dryRun: options?.dryRun });
  ...
  try {
    ...
    log.debug({ quoteId, includes, fees: { combinedMonthly: fees.combinedMonthly, combinedSetup: fees.combinedSetup } }, '🧮 Computed pricing and service flags');
    ...
    if (options?.dryRun) {
      appendModuleLog('hubspot', 'info', 'Dry run executed - no HubSpot or DB mutations', { quoteId, mode, totals: { monthly: fees.combinedMonthly, setup: fees.combinedSetup } });
    ...
    if (mode === 'update' && quote.hubspotDealId) {
      ...
      const newDeal = await svc.createDeal(
        contact.id,
        companyName,
        fees.combinedMonthly,
        fees.combinedSetup,
    ...
    if (!dealId) {
      throw new Error('Failed to create or update HubSpot deal');
    }
    ...
    if (mode === 'create') {
      ...
      appendModuleLog('hubspot', 'info', 'HubSpot create sync completed', { quoteId, dealId, hubspotQuoteId: created.id });
    ...
    log.info({ quoteId, dealId, hubspotQuoteId: quote.hubspotQuoteId, updatedOk }, '✅ HubSpot update sync completed');
    appendModuleLog('hubspot', 'info', 'HubSpot update sync completed', { quoteId, dealId, hubspotQuoteId: quote.hubspotQuoteId, updatedOk });
    const durationMs = Date.now() - start;
    await recordHubspotSync(Boolean(updatedOk), durationMs, Boolean(updatedOk) ? undefined : 'updateQuote returned false');
    ...
    appendModuleLog('hubspot', 'error', 'HubSpot sync failed', { quoteId, action, error: err?.message });
    throw err;
  }
}
