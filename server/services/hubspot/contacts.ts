...
export function createContactsService(request: HubSpotRequestFn) {
  ...
  async function verifyContactByEmail(email: string): Promise<{ verified: boolean; contact?: any }> {
    try {
      const searchBody = {
        filterGroups: [
          {
        ...
        properties: [
          ...
          'phone','mobilephone','hs_phone','phone_number','work_phone','mobile_phone','home_phone','fax','secondary_phone','phone_ext','phonenumber',
        ],
      ...
      if (result.results && result.results.length > 0) {
        ...
        if (contact.associations?.companies?.results?.length > 0) {
          ...
          try {
            const companyResult = await request(`/crm/v3/objects/companies/${companyId}?properties=industry,annualrevenue,numberofemployees,hs_industry_group,monthly_revenue_range,entity_type`);
            if (companyResult) {
              companyData = {
      ...
      if (ownerEmail) {
        ...
        if (ownerId) {
          searchBody = {
            ...
            limit: 20,
            properties: [
              'email','firstname','lastname','company','industry','annualrevenue','numemployees','phone','city','state','country','notes_last_contacted','notes_last_activity_date','hs_lead_status','lifecyclestage','createdate','lastmodifieddate','hubspot_owner_id',
            ],
          };
        } else {
          searchBody = {
  ...
  async function getContactById(contactId: string): Promise<any> {
    try {
      ...
      return await cache.wrap(
        cacheKey,
        async () => {
          return await request(`/crm/v3/objects/contacts/${contactId}?properties=email,firstname,lastname,company,industry,annualrevenue,numemployees,phone,city,state,country,notes_last_contacted,notes_last_activity_date,hs_lead_status,lifecyclestage,createdate,lastmodifieddate`);
        },
  ...
  async function getContactDeals(contactId: string): Promise<any[]> {
    try {
      ...
      return await cache.wrap(
        cacheKey,
        async () => {
          const dealsResponse = await request(`/crm/v4/objects/contacts/${contactId}/associations/deals`);
          ...
          const dealDetails = await Promise.all(
            dealIds.map(async (dealId: string) => {
              try {
                return await request(`/crm/v3/objects/deals/${dealId}?properties=dealname,dealstage,amount,closedate,pipeline,deal_type,hs_object_id`);
              } catch (err) {
  ...
  async function verifyUser(email: string): Promise<{ exists: boolean; userData?: any }> {
    try {
      ...
      if (response.results) {
        const user = response.results.find((owner: any) => owner.email?.toLowerCase() === email.toLowerCase());
        if (user) {
          return {
            exists: true,
            userData: {
