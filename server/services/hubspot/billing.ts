    ...
    try {
      const resp = await request(
        ...
        `&properties=${encodeURIComponent([
          'hs_createdate','hs_lastmodifieddate','hs_object_id','hs_invoice_amount','hs_balance_due','hs_invoice_number','hs_invoice_status','hs_deal_id','hs_deal_name','company_name','hs_company_name','recipient_company_name','billing_contact_name'
        ].join(','))}` +
          ...
          filterGroups: [
            {
              filters: [
                ...
                { propertyName: 'hs_invoice_paid_date', operator: 'GTE', value: new Date(startDate).getTime().toString() },
                { propertyName: 'hs_invoice_paid_date', operator: 'LTE', value: new Date(endDate).getTime().toString() },
              ],
            },
          ],
          properties: [
      ...
      async () => {
        const searchBody: any = {
          filterGroups: [
            {
              filters: [
                { propertyName: 'hs_subscription_status', operator: 'IN', values: ['ACTIVE', 'PAUSED'] },
              ],
            },
          ],
          properties: [
      ...
      async () => {
        const searchBody = {
          filterGroups: [
            {
              filters: [
                { propertyName: 'hs_subscription_id', operator: 'EQ', value: subscriptionId },
                { propertyName: 'hs_invoice_status', operator: 'EQ', value: 'PAID' },
                { propertyName: 'hs_invoice_paid_date', operator: 'GTE', value: new Date(startDate).getTime().toString() },
                { propertyName: 'hs_invoice_paid_date', operator: 'LTE', value: new Date(endDate).getTime().toString() },
              ],
