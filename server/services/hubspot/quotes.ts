...
export function createQuotesService(
  ...
  deps: {
    ...
    HUBSPOT_PRODUCT_IDS: Record<string, string>;
  }
) {
  ...
  async function doesQuoteExist(quoteId: string): Promise<boolean> {
    try {
      const result = await request(`/crm/v3/objects/quotes/${quoteId}`, { method: 'GET' });
      return !!result?.id;
    } catch {
      return false;
    }
  ...
  async function createInitialServiceLineItems(quoteId: string, serviceConfig: any): Promise<void> {
    ...
    if (serviceConfig.includesBookkeeping) {
      ...
      if (bookkeepingPrice > 0) {
        services.push({ price: bookkeepingPrice, productId: PRODUCT.MONTHLY_BOOKKEEPING });
      }
      const bookkeepingSetupFee = serviceConfig.bookkeepingSetupFee || 0;
      if (bookkeepingSetupFee > 0) {
        services.push({ price: bookkeepingSetupFee, productId: PRODUCT.MONTHLY_BOOKKEEPING_SETUP });
      }
    }
    ...
    if (serviceConfig.cleanupProjectFee > 0) {
      services.push({ price: serviceConfig.cleanupProjectFee, productId: PRODUCT.CLEANUP_PROJECT });
    }

    if (serviceConfig.priorYearFilingsFee > 0) {
      services.push({ price: serviceConfig.priorYearFilingsFee, productId: PRODUCT.PRIOR_YEAR_FILINGS });
    }

    if (serviceConfig.includesPayroll && serviceConfig.payrollFee > 0) {
      services.push({ price: serviceConfig.payrollFee, productId: PRODUCT.PAYROLL_SERVICE });
    }

    if (serviceConfig.includesAP) {
      const apProductId = serviceConfig.apServiceTier === 'advanced' ? PRODUCT.AP_ADVANCED_SERVICE : PRODUCT.AP_LITE_SERVICE;
      if (serviceConfig.apFee > 0) services.push({ price: serviceConfig.apFee, productId: apProductId });
    }

    if (serviceConfig.includesAR) {
      const arProductId = serviceConfig.arServiceTier === 'advanced' ? PRODUCT.AR_ADVANCED_SERVICE : PRODUCT.AR_LITE_SERVICE;
      if (serviceConfig.arFee > 0) services.push({ price: serviceConfig.arFee, productId: arProductId });
    }

    if (serviceConfig.includesAgentOfService && serviceConfig.agentOfServiceFee > 0) {
      services.push({ price: serviceConfig.agentOfServiceFee, productId: PRODUCT.AGENT_OF_SERVICE });
    }

    if (serviceConfig.serviceTier === 'Concierge' && serviceConfig.serviceTierFee > 0) {
      services.push({ price: serviceConfig.serviceTierFee, productId: PRODUCT.CONCIERGE_SERVICE_TIER });
    } else if (serviceConfig.serviceTier === 'Guided' && serviceConfig.serviceTierFee > 0) {
      services.push({ price: serviceConfig.serviceTierFee, productId: PRODUCT.GUIDED_SERVICE_TIER });
    }

    if (serviceConfig.qboSubscription) {
      const qboPrice = serviceConfig.qboFee || 60;
      services.push({ price: qboPrice, productId: PRODUCT.MANAGED_QBO_SUBSCRIPTION });
    }

    for (const s of services) {
      await associateProductWithQuote(quoteId, s.productId, s.price, 1, null as any);
    }
  }

  async function fetchExistingLineItems(quoteId: string): Promise<Array<{ id: string; productId: string; price: number; quantity: number; name: string }>> {
    const associations = await request(`/crm/v4/objects/quotes/${quoteId}/associations/line_items`, { method: 'GET' });
    if (!associations?.results?.length) return [];
    const lineItems = await Promise.all(
      associations.results.map(async (association: any) => {
        try {
          const lineItem = await request(`/crm/v3/objects/line_items/${association.toObjectId}?properties=name,price,quantity,hs_product_id,hs_sku`);
          return {
            id: lineItem.id,
  ...
  function analyzeLineItemChanges(
    ...
    for (const existingItem of existingItems) {
      ...
      if (!requiredService) {
        toDelete.push({ id: existingItem.id, productId: existingItem.productId, name: existingItem.name });
      } else if (Math.abs(existingItem.price - requiredService.price) > 0.01) {
        toUpdate.push({ id: existingItem.id, productId: existingItem.productId, oldPrice: existingItem.price, newPrice: requiredService.price });
      }
    }
  ...
  async function executeLineItemChanges(
    ...
    for (const update of changes.toUpdate) {
      try {
        await request(`/crm/v3/objects/line_items/${update.id}`, {
          method: 'PATCH',
          body: JSON.stringify({ properties: { price: update.newPrice.toString() } }),
        });
      } catch {
    ...
    for (const deletion of changes.toDelete) {
      try {
        await request(`/crm/v3/objects/line_items/${deletion.id}`, { method: 'DELETE' });
      } catch {
    ...
    for (const addition of changes.toAdd) {
      try {
        await associateProductWithQuote(quoteId, addition.productId, addition.price, 1, null as any);
      } catch {
        // continue other adds
      }
    }
...
<a href="https://seedfinancial.io/legal/msa-v-2025-07-01">MSA v2025.07.01</a>`;
    ...
    if (includesBookkeeping && !added.has('A')) {
      schedules.push('<a href="https://seedfinancial.io/legal/ssa-v-2025-09-01">SCHEDULE A - BOOKKEEPING v2025.09.01</a>');
      added.add('A');
    }
    if (includesTaas && !added.has('B')) {
      schedules.push('<a href="https://seedfinancial.io/legal/ssb-v-2025-09-01">SCHEDULE B - TAX AS A SERVICE v2025.09.01</a>');
      added.add('B');
    }
    if (includesPayroll) schedules.push('<a href="https://www.seedfinancial.io/legal/ssc-v-2025-09-01">SCHEDULE C - PAYROLL v2025.09.01</a>');
    if (includesAP) schedules.push('<a href="https://www.seedfinancial.io/legal/ssd-v-2025-09-01">SCHEDULE D - ACCOUNTS PAYABLE v2025.09.01</a>');
    if (includesAR) schedules.push('<a href="https://www.seedfinancial.io/legal/sse-v-2025-09-01">SCHEDULE E - ACCOUNTS RECEIVABLE v2025.09.01</a>');
    if (includesAgentOfService) schedules.push('<a href="https://www.seedfinancial.io/legal/ssf-v-2025-09-01">SCHEDULE F - AGENT OF SERVICE v2025.09.01</a>');
  ...
  function generateScopeAssumptions(quoteData: any): string {
    ...
    if (quoteData.serviceTaas || quoteData.includesTaas || quoteData.serviceTaasMonthly) {
      ...
      const numEntitiesText = quoteData.customNumEntities ?? quoteData.numEntities ?? 'Not specified';
      assumptions.push(`• Number of Entities: ${numEntitiesText}`);
      const statesFiledText = quoteData.customStatesFiled ?? quoteData.statesFiled ?? 'Not specified';
    ...
    if (quoteData.serviceCfoAdvisory) {
      ...
      if (quoteData.cfoAdvisoryBundleHours) assumptions.push(`• Bundle Hours: ${quoteData.cfoAdvisoryBundleHours}`);
    }
  ...
  async function createQuote(
    ...
    const scopeAssumptions = quoteData ? generateScopeAssumptions(quoteData) : '';
    ...
    const result = await request('/crm/v3/objects/quotes', { method: 'POST', body: JSON.stringify(quoteBody) });
  ...
  async function updateQuote(
    ...
    const check = await request(`/crm/v3/objects/quotes/${quoteId}`, { method: 'GET' });
    ...
    if (includesBookkeeping && bookkeepingMonthlyFee && bookkeepingMonthlyFee > 0) {
      requiredServices.push({ price: bookkeepingMonthlyFee, productId: PRODUCT.MONTHLY_BOOKKEEPING });
    }
    if (includesBookkeeping && bookkeepingSetupFee && bookkeepingSetupFee > 0) {
      requiredServices.push({ price: bookkeepingSetupFee, productId: PRODUCT.MONTHLY_BOOKKEEPING_SETUP });
    }
    ...
    if (taasPriorYearsFee && taasPriorYearsFee > 0) {
      requiredServices.push({ price: taasPriorYearsFee, productId: PRODUCT.PRIOR_YEAR_FILINGS });
    }
    if (cleanupProjectFee && cleanupProjectFee > 0) {
      requiredServices.push({ price: cleanupProjectFee, productId: PRODUCT.CLEANUP_PROJECT });
    }
    if (includesPayroll && payrollFee && payrollFee > 0) {
      requiredServices.push({ price: payrollFee, productId: PRODUCT.PAYROLL_SERVICE });
    }
    if (includesAP && apFee && apFee > 0) {
      const apProductId = quoteData?.apServiceTier === 'advanced' ? PRODUCT.AP_ADVANCED_SERVICE : PRODUCT.AP_LITE_SERVICE;
      requiredServices.push({ price: apFee, productId: apProductId });
    }
    if (includesAR && arFee && arFee > 0) {
      const arProductId = quoteData?.arServiceTier === 'advanced' ? PRODUCT.AR_ADVANCED_SERVICE : PRODUCT.AR_LITE_SERVICE;
      requiredServices.push({ price: arFee, productId: arProductId });
    }
    if (includesAgentOfService && agentOfServiceFee && agentOfServiceFee > 0) {
      requiredServices.push({ price: agentOfServiceFee, productId: PRODUCT.AGENT_OF_SERVICE });
    }
    if (includesCfoAdvisory && cfoAdvisoryFee && cfoAdvisoryFee > 0) {
      requiredServices.push({ price: cfoAdvisoryFee, productId: PRODUCT.CFO_ADVISORY_DEPOSIT });
    }
    ...
    if (serviceTier === 'Concierge' && serviceTierFee > 0) {
      requiredServices.push({ price: serviceTierFee, productId: PRODUCT.CONCIERGE_SERVICE_TIER });
    } else if (serviceTier === 'Guided' && serviceTierFee > 0) {
      requiredServices.push({ price: serviceTierFee, productId: PRODUCT.GUIDED_SERVICE_TIER });
    }

    if (quoteData?.qboSubscription) {
      const qboPrice = quoteData?.qboFee || 60;
      requiredServices.push({ price: qboPrice, productId: PRODUCT.MANAGED_QBO_SUBSCRIPTION });
    }
    ...
    await request(`/crm/v3/objects/quotes/${quoteId}`, { method: 'PATCH', body: JSON.stringify(updateBody) });
