...
export async function verifyContactHandler(req: Request, res: Response) {
  try {
    ...
    if (!hubSpotService) {
      ...
      return res.json({ success: false, data: { verified: false }, verified: false, error: 'HubSpot integration not configured' });
    }
    ...
    return res.status(500).json({ success: false, error: { message: 'Failed to verify contact', code: getErrorMessage(error) } });
  }
}
...
export async function searchContactsHandler(req: Request, res: Response) {
  try {
    ...
    if (!hubSpotService) {
      return res.status(500).json({ success: false, error: { message: 'HubSpot integration not configured' } });
    }
    ...
    return res.status(500).json({ success: false, error: { message: 'Failed to search HubSpot contacts', code: getErrorMessage(error) } });
  }
}
