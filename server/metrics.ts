...
export async function recordHubspotSync(success: boolean, durationMs: number, failureReason?: string) {
  const redis = await getRedisAsync();
  if (!redis) {
    // In-memory fallback
    if (success) memSuccess++; else memFailure++;
    memDurations.unshift(durationMs);
    if (memDurations.length > 1000) memDurations = memDurations.slice(0, 1000);
    if (!success && failureReason) memLastFailure = { at: new Date().toISOString(), reason: failureReason };
    return;
  }
  ...
  try {
    if (success) {
      ...
      if (failureReason) {
        await cacheRedis.set('metrics:hubspot:sync:lastFailure', JSON.stringify({ at: new Date().toISOString(), reason: failureReason }));
      }
    }
...
export async function getHubspotMetrics() {
  ...
  try {
    ...
    const durations = (durs || []).map((s: string) => parseInt(s, 10)).filter((n: number) => Number.isFinite(n));
